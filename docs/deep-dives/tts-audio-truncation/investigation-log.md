...
>
> **后续（2026-09-21）**：#8 引入的 jj11hh/opus WASM 编码器在本地验证中越界写宿主内存，导致任意协程随机崩溃（TLS 建连/编码器内均有实例），已移除；编码器默认改用 audio_encoder_cgo.go 的 CGo 原生 libopus，Dockerfile 不再需要 `-tags opus_native`
>
> **进展**：
