// http.go 认证域 HTTP 出口：注册/邮箱验证/登录/密码 的 JSON 接口
//
// handler 统一签名 transport.HandlerFunc（成功 transport.WriteJSON，失败返回领域错误）
// 受保护端点经 AccessRequired 校验后从 context 取 claims，签名与普通端点一致
package authn
...
// handleRegister POST /api/v1/auth/register 注册接口
...
func (h *Handler) PasswordResetRequest(w http.ResponseWriter, r *http.Request) error {
...
func (h *Handler) PasswordReset(w http.ResponseWriter, r *http.Request) error {
	var req struct {
		// Token 一次性重置令牌
		Token string `json:"token"`
		// NewPassword 新密码
...
func (h *Handler) EmailBind(w http.ResponseWriter, r *http.Request) error {
	claims, ok := ClaimsFrom(r.Context())
	if !ok {
		return ErrAccessTokenInvalid
	}
...
func (h *Handler) PasswordChange(w http.ResponseWriter, r *http.Request) error {
	claims, ok := ClaimsFrom(r.Context())
	if !ok {
		return ErrAccessTokenInvalid
	}
