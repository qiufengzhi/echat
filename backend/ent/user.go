...
type User struct {
	...
	// 联系邮箱，可空（未绑定时 NULL），登录凭据在 identities，本列仅展示与找回
	Email *string `json:"email,omitempty"`
	// 用户名句柄，本地账号登录标识，3-20 位字母数字 _ -
	Username string `json:"username,omitempty"`
	// 展示昵称，可随时修改且允许重名
	DisplayName string `json:"display_name,omitempty"`
	// 头像地址，可空
	AvatarURL *string `json:"avatar_url,omitempty"`
	// 账户状态机：pending / active / suspended / deleted
	Status user.Status `json:"status,omitempty"`
	// 令牌版本，封禁/改密/重置时 +1，使旧 access 失效的关键
	TokenVersion int `json:"token_version,omitempty"`
	// 账户主凭据的 argon2id 哈希，local 与 email 登录共用同一份密码
...
func (*User) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case user.FieldEmail, user.FieldUsername, user.FieldDisplayName, user.FieldAvatarURL, user.FieldStatus, user.FieldPasswordHash:
